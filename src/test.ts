...
const registerTest = (name: string, fn: TestFn, skipped = false, only = false): void => {
  currentSuite().tests.push({ name, fn, skipped, only });
};

export const test: TestFnWithModifiers = Object.assign((name: string, fn: TestFn) => registerTest(name, fn), {
